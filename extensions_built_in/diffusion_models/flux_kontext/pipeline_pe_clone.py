...
def prepare_img_ids_new_single_patch(
        batch_size: int,
        packed_latent_height_layout: int,
        packed_latent_width_layout: int,
        packed_latent_height_img: int,
        packed_latent_width_img: int,
        position_list: list,     # 单个 patch 的全 src/dst 块
):
    """
    处理单个 ref patch
    返回:
       img_ids_layout: [1, HW, 3]
    """
    ...
    # ===== 对这个 patch 的每个 block 做 TPS warp =====
    for src_pts_list, dst_pts_2_list in [position_list]:
        ...
        tps.estimateTransformation(dst_pts_2.reshape(1, -1, 2),
        ...
        tgt_crop = img_ids_np_layout[y_min:y_max + 1, x_min:x_max + 1]

        # 尺寸不一致裁剪
        if warped_crop.shape != tgt_crop.shape:
            warped_crop = warped_crop[:tgt_crop.shape[0], :tgt_crop.shape[1], :]

        img_ids_np_layout[y_min:y_max + 1, x_min:x_max + 1] = warped_crop

    # ===== reshape to [1, HW, 3] =====
    img_ids_final = torch.from_numpy(img_ids_np_layout).float()
    img_ids_final = img_ids_final.reshape(1, -1, 3)

    return img_ids_final
...
def prepare_img_ids_new(batch_size: int, packed_latent_height_layout: int, packed_latent_width_layout: int,packed_latent_height_img:int, packed_latent_width_img:int,position_lists: list):
    all_img_ids_layout = []
    for position_list in [position_lists]: #bs=1
        img_ids = torch.zeros(packed_latent_height_img, packed_latent_width_img, 3)
        img_ids[..., 1] = img_ids[..., 1] + torch.arange(packed_latent_height_img)[:, None]
        img_ids[..., 2] = img_ids[..., 2] + torch.arange(packed_latent_width_img)[None, :]
        ...
        for src_pts_list, dst_pts_2_list in position_list:
            ...
            warped_img = tps.warpImage(img_ids_np)
            # M = cv2.getPerspectiveTransform(src_pts, dst_pts_2)
            # warped_img = cv2.warpPerspective(
            #     img_ids_np,
            #     M,
            #     (img_ids_np.shape[1], img_ids_np.shape[0]),
            ...
            warped_crop = warped_img[y_min:y_max+1, x_min:x_max+1]
            target_crop = img_ids_np_layout[y_min:y_max+1, x_min:x_max+1]

            # 如果 warp 出来的 patch 尺寸比目标大，就截掉多余边界
            if warped_crop.shape != target_crop.shape:
                warped_crop = warped_crop[:target_crop.shape[0], :target_crop.shape[1], :]
            img_ids_np_layout[y_min:y_max+1, x_min:x_max+1] = warped_crop
    ...
    img_ctrl_ids = einops.repeat(ctrl_ids, "h w c -> b (h w) c", b=batch_size)
    img_ids_batch = torch.stack(all_img_ids_layout, dim=0)
    img_ids_batch = img_ids_batch.view(batch_size, -1, 3)
    img_ids = torch.cat((img_ids, img_ctrl_ids, img_ids_batch), dim=1)
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def _pack_latents(latents, batch_size, num_channels_latents, height, width):
        ...
        latents = latents.permute(0, 2, 4, 1, 3, 5) #(batch_size, height // 2, width // 2, num_channels_latents,2, 2)
